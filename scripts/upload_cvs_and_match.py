...
This mimics the API workflow: register -> get token -> upload CVs -> save predictions.
"""
...
RESULTS_DIR = Path(__file__).parent.parent / "tests" / "test_predicted_results"
...
def save_prediction_results(cv_filename: str, prediction_data: Dict[str, Any]) -> None:
    """
    Save prediction results to a JSON file.

    Args:
        cv_filename: Original CV filename (e.g., "TEACHER_12467531.pdf")
        prediction_data: Full response from the match endpoint
    """
    # Create results directory if it doesn't exist
    RESULTS_DIR.mkdir(exist_ok=True, parents=True)

    # Generate output filename (replace .pdf with .json)
    output_filename = cv_filename.replace('.pdf', '.json')
...
def main():
    ...
    if len(cv_files) > 10:
        ...
        user_input = input(f"Process all {len(cv_files)} CVs? (y/n, or enter number to limit): ")
    ...
    for idx, cv_path in enumerate(cv_files, 1):
        ...
        result = upload_cv_and_match(cv_path, access_token)

        if result:
            # Save prediction results
            save_prediction_results(cv_path.name, result)
            successful += 1
        else:
            failed += 1
